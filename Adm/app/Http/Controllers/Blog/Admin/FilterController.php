...
class FilterController extends AdminBaseController
{
    ...
    public function groupEdit(BlogGroupFilterAddRequest $request, $id){
    	if (empty($id)) {
    		return back()->withErrors(['msg' => "Записть [{$id}] не найдена!"]);
    	}
    	if ($request->isMethod('post')){
    ...
    /**Show All Attribute for Filters table->attribute_values*/
        ...
        if ($request->isMethod('post')){
            ...
            if ($uniqueName) {
                return redirect('/admin/filter/attrs-add')
                    ->withErrors(['msg' => 'Такое название фильтра уже сущетсвует.'])
                    ->withInput();
            }
            $data = $request->input();
    ...
    public function attrDelete($id){
        ...
        if ($delete) {
            ...
            return back()->withErrors(['msg' => "Ошиба удаления"]);
        }
    }
}
