					...
					<form action="{{url('/admin/currency/edit', $currency->id)}}" method="post" data-toggle="validator">
						...
						<div class="box-body">
							...
							<div class="form-group has-feedback">
								...
								<input type="text" name="value" class="form-control" id="title" value="{{$currency->value, old('value')}}" title="если это базовая валюта поставьте 1, то курс к базовой валюте." required data-error="Допускаются цифры и десятичная точка" pattern="^[0-9.]{1,}">
								<span class="glyphicon form-control-feedback" aria-hidden="true"></span>
							...
							<div class="form-group has-feedback">
								...
								@if ($currency->base) checked @endif>Базовая валюта
								</label>
							</div>
						</div>
